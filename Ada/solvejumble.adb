...
procedure solveJumble is
    ...
    -- wordStr - The origional character string entered
    -- anagramsList - A wordsArray object containing the anagrams
    -- dictionaryList - A set object containing words in a dictionary
    procedure findAnagram(numAnagrams : in integer; wordStr : in unbounded_string; anagramsList : in wordsArray; dictionaryList : in set) is 
        -- numAnagramsFound - integer to keep track of number of anagrams found for each input string
        numAnagramsFound : integer := 0;
    begin
        ...
        for i in 1 .. numAnagrams loop
            if contains(dictionaryList, anagramsList(i)) then
                if numAnagramsFound > 0 then
                    put(", ");
                end if;
                put(anagramsList(i));                
                numAnagramsFound := numAnagramsFound +1;          
            end if;
        end loop;
        if numAnagramsFound > 0 then
            put(" - ");
        end if;
        put(numAnagramsFound, width => 1);
        put(" anagram(s) found from jumble: ");
        put(wordStr);
        new_line;
...
begin
    ...
    for i in 1 .. numWords loop
        ...
        begin
            ...
            findAnagram(numAnagrams, wordsList(i), anagramsList, dictionaryList);
        end;
    end loop;
